...
//        }
    ...
    public void rotateBlock() {
        ...
        if (block.getRightEdge() >= gridColumns) {
            block.setX(gridColumns - block.getWeight());
        }
    ...
    public void restoreGame(GameAreaEncode gae){
        ...
        for (int r = 0; r < gridRows; r++) {
            for (int c = 0; c < gridColumns; c++) {
                if (backgroundInt[r][c] == -1) {
                    background[r][c] = null;
                } else {
                    for (int i = 0; i < arrColor.length; i++) {
                        if( backgroundInt[r][c] == i){
                            background[r][c] = arrColor[i];
                            break;
                        }
                    }
    ...
    public void encode(GameAreaEncode gae) {
        // chuyen background sang mang int
        int[][] backgroundInt = new int[gridRows][gridColumns];
        Color[] arrColor = block.getAvailableColors();
        ...
        for (int r = 0; r < gridRows; r++) {
            for (int c = 0; c < gridColumns; c++) {
                if (background[r][c] == null) {
                    ...
                    Color myColor = background[r][c];
                    int index = 0;
                    for (Color item : arrColor) {
                        if (item.equals(myColor)) {
                            backgroundInt[r][c] = index;
                            break;
                        }
                        index++;
                    }
